...
export default async function handler(request,response){
  ...
  try{return response.status(200).json(await loadRunningPlaces({lat:latitude,lon:longitude},radius));}catch(error){console.error(error);return response.status(503).json({error:{code:error.code||'PLACE_SEARCH_FAILED',message:'Nearby running spots are temporarily unavailable.'}});}
}
