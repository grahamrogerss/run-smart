...
export async function createRoute(input,dependencies={loadMapData}){
  ...
  if(!Number.isFinite(latitude)||latitude<-90||latitude>90||!Number.isFinite(longitude)||longitude<-180||longitude>180)throw Object.assign(new Error('Valid latitude and longitude are required.'),{status:400});
  if(!Number.isFinite(distanceMiles)||distanceMiles<.5||distanceMiles>20)throw Object.assign(new Error('Distance must be between 0.5 and 20 miles.'),{status:400});
  if(!PRESETS.has(preset))throw Object.assign(new Error('Unknown route preset.'),{status:400});
  const started=performance.now(),origin={lat:latitude,lon:longitude},target=distanceMiles*M_PER_MI,mapStart=performance.now(),mapData=await dependencies.loadMapData(bboxFor(origin,target)),graphStart=performance.now(),graph=buildGraph(mapData.elements),routeStart=performance.now();
  if(graph.nodes.size<10)throw Object.assign(new Error('Not enough runnable paths were found near this location.'),{status:422});
  const route=findLoop(graph,origin,target,preset);if(!route)throw Object.assign(new Error('No connected loop could be built here.'),{status:422});
